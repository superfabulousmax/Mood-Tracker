...
import { depressionQuestions } from '../data/questions.js';
...
export default function DepressionPage() {
  ...
  useEffect(() => {
    setEntries(loadEntries().filter((entry) => entry.type === 'depression'));
  }, []);
  ...
  const handleSave = (entry) => {
    ...
    saveEntries(updated);
    setEntries(updated.filter((item) => item.type === 'depression'));
  };
  ...
  const handleDelete = (entryId) => {
    ...
    setEntries(rawEntries.filter((entry) => entry.type === 'depression'));
  };

  return (
    <div>
      <QuestionnaireForm title="Depression" questions={depressionQuestions} onSave={handleSave} />
      <div className="card">
        <h2 className="heading">Saved Depression Entries</h2>
        {entries.length === 0 ? (
          <p className="note">No depression entries have been saved yet.</p>
        ) : (
